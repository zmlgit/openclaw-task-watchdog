...
### Fixed
- **P1-4**: Escalation target now uses recorded main session key instead of bare string `"main"`. Falls back to `extractParentSessionKey` if no main session seen yet.
- **P1-6**: Subagent sessions now use a higher consecutive tool call threshold. New config `subagentConsecutiveThreshold` (default: 15 or `consecutiveToolCallThreshold * 3`).
- **P1-5**: `gateway_start` immediate wake now uses `lastMainSessionKey` (fallback `"main"`) for safer routing after restart.
